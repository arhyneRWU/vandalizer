...
export const MODULES: ModuleDefinition[] = [
  {
    ...
    description: 'Welcome to the Vandal Workflow Architect certification. By the end of this program, you\'ll earn an official VWA credential recognizing your ability to design and deploy AI-powered workflows for research administration. This first module starts with the big picture \u2014 what AI is, why it has landed in research administration, and how this course helps you \u2014 before touching any tool. No technical skills required, ever: every module is bite-sized (10\u201325 minutes), and you can leave and pick up exactly where you stopped.',
    objectives: [
  ...
  },
  {
    ...
    lessons: [
      ...
      },
      {
        ...
        content: 'The most common and most effective workflow pattern in research administration has three phases:\n\n1. **Extract** \u2014 Pull structured data from the document. This is your Extraction step with a well-designed Extraction. The output is clean, consistent JSON.\n\n2. **Reason** \u2014 Analyze the extracted data. This might be a Prompt step that summarizes findings, a Code Execution step that computes totals, or both. The output is analysis or computed results.\n\n3. **Deliver** \u2014 Produce something useful. A formatted report, a CSV export, a compliance checklist, or structured data ready for your tracking system.\n\nThis pattern works because it separates concerns. If your final report is wrong, you can check: did the extraction get the right data? (Check step 1\'s output.) Did the analysis interpret it correctly? (Check step 2.) You can fix the broken step without rebuilding the whole workflow.\n\n**The Extract phase is optional, and task-dependent.** "Extract" means *get what you need out of the document*, and the Extraction task is one way to do that \u2014 the right way when you want the same fixed fields from every document, when the document is long enough that the reasoning step should not see all of it, or when you need values you can check one by one. For a short or context-heavy document \u2014 a two-page amendment letter, a sponsor email, a memo where the answer depends on the whole narrative \u2014 an Extraction step in front adds a hop without adding reliability. Send the whole file to the Prompt instead: the first step after the Document trigger receives the workflow\'s documents automatically, and any later step can choose **Workflow Documents** or **Selected Document** as its input source. Teams that have tried both report that for those documents the direct Prompt gives better answers. Reason and Deliver stay the same either way.',
        variant: 'concept',
        diagram: 'extract-reason-deliver',
        knowledgeCheck: {
          question: 'When is it better to skip the Extraction step and send the whole document straight to a Prompt?',
          options: [
            { text: 'Never \u2014 a workflow should always start with an Extraction task', correct: false, explanation: 'Extraction is optional and task-dependent. It earns its place when you need fixed fields or checkable values; a short or context-heavy document is often read better by a Prompt that sees the whole file.' },
            { text: 'When the document is short or the answer depends on its full context, and you don\'t need fixed fields', correct: true, explanation: 'Correct! A two-page letter or a memo has no field list to pull and its meaning lives in the whole text. The first step after the Document trigger receives the document automatically \u2014 no Extraction needed. Reason and Deliver still follow.' },
            { text: 'When the document is very long', correct: false, explanation: 'Long documents are where Extraction earns its keep: it narrows what the reasoning step sees and gives you values you can verify one by one.' },
            { text: 'When you need machine-readable output for a spreadsheet', correct: false, explanation: 'Consistent, structured fields are exactly what the Extraction task is for. That is the case where you keep it.' },
          ],
        },
      },
      {
  ...
  },
  {
    ...
    lessons: [
      ...
      },
      {
        ...
        content: 'The Prompt node is one of the most powerful tools in your workflow. It sends the previous step\'s output to the LLM along with your custom prompt, and returns the LLM\'s response.\n\nUse it to:\n\u2022 Summarize extracted data\n\u2022 Compare and analyze\n\u2022 Generate recommendations\n\u2022 Transform formats\n\nThe key insight is that extraction gives you structured data, and prompts let you reason over that data.\n\nA Prompt does not need an Extraction in front of it. As the first step it receives the workflow\'s documents directly; later in a chain, set the step\'s input source to **Workflow Documents** or **Selected Document** to hand it the whole file. For a short or context-heavy document that is often the better design \u2014 this module\'s lab uses Extraction first because a 40-page agreement with fixed terms is the case Extraction is built for.',
        variant: 'concept',
        knowledgeCheck: {
      ...
      },
      {
        ...
        content: '*You\'ve seen this principle before in Module 3 \u2014 that\'s intentional. Spaced repetition builds retention. Let\'s revisit it and see how it applies when chaining multiple steps.*\n\nA common mistake is trying to do everything in one big prompt. Instead, separate reading (getting the facts out of the document) from reasoning (drawing conclusions from those facts). This makes each step simpler, more reliable, and easier to debug. If your final output is wrong, you can check: did the reading step get the right data? Or did the reasoning step misinterpret it? This separation is what makes workflows more reliable than one-shot prompting.\n\nThe principle is *separation of concerns*, not "Extraction goes first." For a long document with fixed fields, the reading step is an Extraction task. For a short or context-heavy document, the reading step can be a Prompt that takes the whole file \u2014 the important part is that the step that reads and the step that judges are not the same step.',
        variant: 'insight',
      },
      {
