...
| Workflows | owner; team member for team workflows; examiners / admins while a verification request is open | owner; team `owner` / `admin` | All CRUD and execution delegated through `get_authorized_workflow`. Document selections pass through `_authorize_documents`. Steps/tasks resolve parent workflow for authz. | Audited |
